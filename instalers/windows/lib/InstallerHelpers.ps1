...
function Test-IsPythonPathCandidate {
    ...
    if ($normalized -match '\\AppData\\Local\\Microsoft\\WindowsApps\\') { return $false }
...
function Initialize-ProjectVenv {
    ...
    $req = Join-Path $ProjectRoot $cfg.RequirementsFile
    if (-not (Test-Path $req)) {
        throw "Missing $($cfg.RequirementsFile) at $ProjectRoot"
    }
    Write-InstallLog 'Installing Python packages (may take 5-15 minutes on first run)...'
    & $venvPython -m pip install --upgrade pip wheel setuptools
    if ($LASTEXITCODE -ne 0) { throw 'pip upgrade failed.' }
    & $venvPython -m pip install -r $req
    if ($LASTEXITCODE -ne 0) { throw "pip install -r $($cfg.RequirementsFile) failed." }
...
echo Starting $($cfg.AppName)...
echo Close this window to stop the app.
".venv\Scripts\python.exe" -m streamlit run $($cfg.AppScript)
pause
"@
    Set-Content -Path (Join-Path $ProjectRoot $cfg.StartBatName) -Value $startBat -Encoding ASCII
    Write-InstallLog "Wrote launcher: $($cfg.StartBatName)"
}

function Start-AnisotropiaApp {
    param(
        [string]$VenvPython,
        [string]$ProjectRoot
    )
    $cfg = $script:AnisotropiaConfig
    $app = Join-Path $ProjectRoot $cfg.AppScript
    if (-not (Test-Path $app)) {
        throw "Missing $($cfg.AppScript) at $ProjectRoot"
    }
    Write-InstallLog 'Starting Anisotropia (browser should open)...'
